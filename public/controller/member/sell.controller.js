...
exports.showSellProductAllGET= function(req,res){
    ...
    FabricQuery(request).then((resolvedData)=>{
        return res.render('member/showSellProduct',{items:resolvedData});
    }).catch((e)=>{
        console.log(e);
    });
};
...
exports.showSellOneProduct = function(req,res){
    ...
    //판매 물품 키값으로 조회
    var request = makeRequset('querySellProductOne',uuid);

    FabricQuery(request).then((resolvedData)=>{
        return res.render('member/showSellProductOne',{items:resolvedData});        
    }).catch((e)=>{
        console.log(e);
    })
