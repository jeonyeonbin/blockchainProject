...
class FBClient extends FabricClient{
    ...
    submitTransaction(requestData){
        ...
        return channel.sendTransactionProposal(requestData).then(function(results){
            ...
            if(isProposalGood){
                console.log('Successfully sent Proposal and received ProposalResponse: Status: %s message : %s',proposalResponses[0].response.status,proposalResponses[0].response.message);
                var request={
                ...
                let txPromise = new Promise((resolve,reject)=>{
                    ...
                    event_hub.registerTxEvent(transaction_id_string,(tx,code)=>{
                        ...
                        if(code!=='VALID'){
                            ...
                            console.log('The transaction has benn committed on peer'+ event_hub._ep._endpoint.addr);
                            resolve(return_status);
                        }
                    }, (err) => {
                        console.log(err);
                        reject(new Error('There was a problem event_hub'+err));
                    });
                });
                ...
                console.error('Faild to send Proposal or recieve valid response not status in 200');
            ...
            if(results && results[0] && results[0].status === 'SUCCESS'){
                ...
                console.error('Failed to order transaction Error code '+response.status);
            }
            if(results && results[1] && results[1].event_status ==='VALID'){
                console.log('Sucessfully commited the change to the ledger by the peer');
            }else{
